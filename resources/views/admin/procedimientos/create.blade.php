...
@section('content')
    <div class="p-6 bg-gray-50 min-h-screen">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Nuevo Procedimiento</h1>
        </div>

        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-lg">
            <form action="{{ route('admin.procedimientos.store') }}" method="POST" class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <select name="area" required class="w-full border rounded-lg px-3 py-2 text-sm @error('area') border-red-500 @enderror">
                        ...
                        @foreach(['emergencia','uti','internacion','cirugia','neonato'] as $area)
                ...
                <div>
                    ...
                    <textarea name="descripcion" rows="3"
                        class="w-full border rounded-lg px-3 py-2 text-sm">{{ old('descripcion') }}</textarea>
                </div>

                <div class="flex items-center gap-2">
                    <input type="checkbox" name="activo" value="1" id="activo" {{ old('activo', true) ? 'checked' : '' }}>
