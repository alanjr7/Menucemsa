...
<div class="p-6 bg-gray-50 min-h-screen">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Editar cuna — {{ $cuna->nombre }}</h1>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-2xl">
        <form method="POST" action="{{ route('admin.neonato.cunas.update', $cuna->id) }}">
            @csrf @method('PUT')
            <div class="grid grid-cols-1 gap-4">

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nombre <span class="text-red-500">*</span></label>
                    <input type="text" name="nombre" value="{{ old('nombre', $cuna->nombre) }}"
                        class="w-full border rounded-lg px-3 py-2 text-sm @error('nombre') border-red-400 @enderror" autocomplete="off">
                    @error('nombre')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Código <span class="text-red-500">*</span></label>
                    <input type="text" name="codigo" value="{{ old('codigo', $cuna->codigo) }}"
                ...
                <div>
                    ...
                    <input type="number" name="precio_por_hora" value="{{ old('precio_por_hora', $cuna->precio_por_hora) }}"
                        step="0.01" min="0"
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="activa" id="activa" value="1" {{ $cuna->activa ? 'checked' : '' }}
