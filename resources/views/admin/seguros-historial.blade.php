    ...
    <div class="w-full p-8 bg-[#f8fafc] min-h-screen font-sans antialiased">
        ...
        <div class="flex justify-between items-start mb-8">
            <div>
                ...
                <p class="text-slate-500 text-[15px] font-medium mt-1 ml-11">Registro de autorizaciones y rechazos de
                    seguros</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-white p-6 rounded-[20px] shadow-sm border border-slate-100 text-center">
                ...
                <p class="text-slate-800 text-[32px] font-black tracking-tighter">Bs.
                    {{ number_format($stats['monto_total_cubierto'] ?? 0, 2) }}</p>
            </div>
            <div class="bg-white p-6 rounded-[20px] shadow-sm border border-slate-100 text-center">
                <p class="text-slate-400 text-[13px] font-medium mb-1">Total Paciente</p>
                <p class="text-slate-800 text-[32px] font-black tracking-tighter">Bs.
                    {{ number_format($stats['monto_total_paciente'] ?? 0, 2) }}</p>
            </div>
        </div>
            ...
            <form method="GET" action="{{ route('admin.seguros.historial') }}" class="flex flex-wrap items-end gap-4">
                <div class="flex-1 min-w-[200px]">
                    ...
                    <input type="date" name="fecha_inicio" value="{{ request('fecha_inicio') }}"
                        class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-50 outline-none">
                </div>
                <div class="flex-1 min-w-[200px]">
                ...
                <div class="flex-1 min-w-[200px]">
                    ...
                    <input type="text" name="paciente" value="{{ request('paciente') }}" placeholder="Nombre o CI"
                        class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-50 outline-none">
                </div>
                <div class="flex-1 min-w-[200px]">
                    ...
                    <select name="seguro_id"
                        ...
                        @foreach ($seguros as $seguro)
                            <option value="{{ $seguro->id }}"
                                {{ request('seguro_id') == $seguro->id ? 'selected' : '' }}>{{ $seguro->nombre_empresa }}
                            </option>
                        @endforeach
                    </select>
                </div>
                <div class="flex-1 min-w-[150px]">
                    ...
                    <select name="estado"
                        class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-50 outline-none">
                        <option value="">Todos</option>
                        <option value="autorizado" {{ request('estado') == 'autorizado' ? 'selected' : '' }}>Autorizado
                        </option>
                ...
                <div class="flex gap-2">
                    <button type="submit"
                        class="bg-[#0061df] hover:bg-blue-700 text-white px-6 py-3 rounded-xl text-sm font-bold shadow-sm transition-all">
                        Filtrar
                    </button>
                    <a href="{{ route('admin.seguros.historial') }}"
                        class="bg-gray-100 hover:bg-gray-200 text-slate-700 px-6 py-3 rounded-xl text-sm font-bold transition-all">
                        Limpiar
                    </a>
                </div>
            </form>
        ...
        <div class="bg-white rounded-[24px] border border-slate-100 shadow-sm overflow-hidden mb-6">
            <div class="p-8 border-b border-slate-50 flex justify-between items-center">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('admin.seguros.historial.excel', request()->except('page')) }}"
                        class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-[12px] font-bold shadow-sm transition-colors flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path
                    ...
                    <tbody class="text-[14px] divide-y divide-slate-50">
                        @forelse($historial as $registro)
                            <tr class="hover:bg-slate-50/50 transition-all">
                                ...
                                <td class="px-6 py-4 font-medium text-slate-800">Bs.
                                    {{ number_format($registro->total_calculado, 2) }}</td>
                                <td class="px-6 py-4 font-medium text-green-600">Bs.
                                    {{ number_format($registro->seguro_monto_cobertura ?? 0, 2) }}</td>
                                <td class="px-6 py-4 font-medium text-orange-600">Bs.
                                    {{ number_format($registro->seguro_monto_paciente ?? 0, 2) }}</td>
                                <td class="px-6 py-4">
                                    @if ($registro->seguro_estado === 'autorizado')
                                        <span
                                            class="bg-green-50 text-green-600 border border-green-100 px-3 py-1.5 rounded-lg text-[12px] font-bold">Autorizado</span>
                                    @else
                                        <span
                                            class="bg-red-50 text-red-600 border border-red-100 px-3 py-1.5 rounded-lg text-[12px] font-bold">Rechazado</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-slate-500">{{ $registro->seguroAutorizadoPor?->name ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4">
                                    <button onclick="verCargos('{{ $registro->id }}')"
