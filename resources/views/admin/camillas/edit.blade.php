...
@section('content')
    <div class="p-6 bg-gray-50 min-h-screen">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Editar camilla: {{ $camilla->nombre }}</h1>

        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-2xl">
            <form method="POST" action="{{ route('admin.camillas.update', $camilla) }}">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-4">
                    <div>
                        ...
                        <input type="text" name="nombre" value="{{ old('nombre', $camilla->nombre) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="codigo" value="{{ old('codigo', $camilla->codigo) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="precio_por_hora" value="{{ old('precio_por_hora', $camilla->precio_por_hora) }}"
                            step="0.01" min="0"
                    ...
                    <div>
                        ...
                        <select name="area" class="w-full border rounded-lg px-3 py-2 text-sm @error('area') border-red-400 @enderror">
                            <option value="emergencia" {{ old('area', $camilla->area) === 'emergencia' ? 'selected' : '' }}>Emergencia</option>
                            <option value="uti" {{ old('area', $camilla->area) === 'uti' ? 'selected' : '' }}>UTI</option>
                        </select>
                        @error('area')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex items-center gap-2">
                        <input type="checkbox" name="activa" id="activa" value="1"
                            {{ old('activa', $camilla->activa) ? 'checked' : '' }}
                ...
                <div class="mt-6 flex gap-3 justify-end">
                    ...
                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">Actualizar</button>
                </div>
            </form>
        </div>
    </div>
