...
<div class="w-full p-6 bg-gray-50/50 min-h-screen">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Detalles de Cita Quirúrgica</h1>
        ...
        <div class="flex gap-3">
            ...
            @if($cita->estado === 'programada')
                <a href="{{ route('quirofano.edit', $cita) }}" class="flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium transition-colors">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-bold text-gray-800">{{ $cita->paciente->nombre }}</h2>
                <p class="text-sm text-gray-500">CI: {{ $cita->paciente->ci }} | {{ $cita->fecha->format('d/m/Y') }}</p>
            </div>
            <div class="flex items-center gap-4">
                <span class="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium {{ $cita->estado === 'programada' ? 'bg-blue-100 text-blue-800' : ($cita->estado === 'en_curso' ? 'bg-amber-100 text-amber-800' : ($cita->estado === 'finalizada' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800')) }}">
                ...
                @if($cita->estado === 'programada')
                    <button onclick="iniciarCirugia({{ $cita->id }})" class="flex items-center px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium transition-colors">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @if($cita->estado === 'en_curso')
                    <button onclick="finalizarCirugia({{ $cita->id }})" class="flex items-center px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium transition-colors">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                ...
                @if($cita->estado !== 'finalizada')
                    <button onclick="cancelarCita({{ $cita->id }})" class="flex items-center px-4 py-2 border border-red-300 text-red-700 rounded-lg hover:bg-red-50 font-medium transition-colors">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
