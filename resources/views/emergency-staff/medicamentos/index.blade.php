...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl sm:text-3xl font-bold">Inventario - Emergencia</h1>
        </div>
    ...
    <form method="GET" class="flex flex-col sm:flex-row gap-2 sm:gap-3 mb-4">
        ...
        <div class="flex gap-2">
            ...
            @if(request('buscar') || request('tipo'))
                <a href="{{ route('emergency-staff.medicamentos.index') }}" class="flex-1 sm:flex-none text-center bg-gray-200 text-gray-700 px-4 py-2 rounded text-sm">Limpiar</a>
            @endif
        </div>
    </form>
    ...
    <div class="sm:hidden space-y-3">
        @forelse ($medicamentos as $med)
            @foreach ($med->lotes as $lote)
                @php $stock = $lote->stocks->first(); @endphp
                @if ($stock)
                    <a href="{{ route('emergency-staff.medicamentos.show', $med) }}"
