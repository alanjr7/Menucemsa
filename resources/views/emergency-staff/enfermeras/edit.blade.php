...
<div class="p-6 bg-gray-50/50 min-h-screen">
    ...
    <div class="mb-8">
        ...
        <p class="text-sm text-gray-500">Actualizar datos de {{ $enfermera->user?->name ?? 'la enfermera' }}</p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 max-w-2xl">
        <form action="{{ route('emergency-staff.enfermeras.update', $enfermera) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="tipo" id="tipo" value="{{ old('tipo', $enfermera->tipo) }}"
                        class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-red-500 focus:ring-2 focus:ring-red-200 transition outline-none"
                        placeholder="Ej: Enfermera de Emergencia, Enfermera Jefe">
                </div>

                <!-- Contraseña (opcional) -->
                <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
                    <p class="text-sm text-yellow-800 font-medium mb-3">Cambiar Contraseña (opcional)</p>
