...
class NeonatoController extends Controller
{
    ...
    public function storeEvaluacion(Request $request, Neonato $neonato): RedirectResponse|JsonResponse
    {
        $request->validate([
            'observaciones' => ['nullable', 'string'],
        ...
        DB::transaction(function () use ($request, $neonato) {
            ...
            foreach ($request->input('items', []) as $item) {
                ...
                if ($item['tipo'] === 'medicamento' || $item['tipo'] === 'insumo') {
                    $stock = AlmacenStock::where('ubicacion', 'neonato')
    ...
    {
        $procedimientos = Procedimiento::where('area', 'neonato')
            ->where('activo', true)
            ->orderBy('nombre')
            ->paginate(30);
